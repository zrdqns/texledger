...
- **`src/components/ui/sidebar.tsx`** (pasa a client component): logo TexLedger + subtítulo, label de sección "MENÚ", ítems con icono lucide (LayoutDashboard, Layers, ShoppingCart, Wallet, Users, Bell) y **estado activo** (pill `bg-primario` texto blanco) vía `usePathname().startsWith(href)`; `print:hidden` se conserva.
...
- **`src/components/ui/estilos.ts`** — consts literales completas compartidas: `btnPrimario`, `btnSecundario`, `btnPeligroTexto`, `input`, `labelCampo`, `card`, `cardInteractiva`, `badgeOk`, `badgePeligro`, `badgeAcento`, `badgeNeutro`, `tabla`, `theadFila`, `thCelda`, `tdBase`, `linkSuave`, `tituloPagina`, `subtituloSeccion`. Sustituyen los className repetidos en las ~30 rutas y forms.
