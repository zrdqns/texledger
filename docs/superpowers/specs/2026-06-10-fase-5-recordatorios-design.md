...
- **`recordatorios`**: id, tipo text not null check in (`'factura'`,`'pago_pendiente'`,`'factura_sin_declarar'`), descripcion text not null, fecha_objetivo date not null, estado text not null default `'pendiente'` check in (`'pendiente'`,`'cumplido'`,`'vencido'`), factura_id uuid null references facturas(id), created_at/updated_at + trigger `set_updated_at`.
...
`ActionResult` + Zod; lecturas `createClient` (RLS), escrituras `createAdminClient`. Sin RPC salvo `generar_notificaciones`.

- **Recordatorios:** `listarRecordatorios()` (order fecha_objetivo asc), `crearRecordatorio` (schema: tipo enum, descripcion min 1, fecha_objetivo YYYY-MM-DD, factura_id uuid opcional), `marcarCumplido(id)` (estado='cumplido'), `eliminarRecordatorio(id)` (delete).
- **Notificaciones:** `listarNotificaciones()` (no leídas primero, luego por created_at desc, límite 50), `contarNoLeidas()` (count head), `marcarLeida(id)`, `marcarTodasLeidas()`.
...
- **`/recordatorios`**: dos secciones.
  ...
  - **Recordatorios:** tabla (tipo, descripción, fecha objetivo con `formatFechaBogota`, estado con color — vencido rojo / pendiente zinc / cumplido esmeralda, factura vinculada si hay, acciones Cumplir/Eliminar con confirm) + link a **`/recordatorios/nuevo`**.
